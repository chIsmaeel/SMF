namespace SMF.SourceGenerator.Abstractions.Templates.TypeTemplates.MemberTemplates.PropertyTemplates;
...
public record PropertyTemplate(string Type, string IdentifierName) : TypeMemberTemplate(IdentifierName)
{
    ...
    private void DefaultPropertyComment()
    {
        ...
        tempComment.Append(FirstAccessor.Pascalize()).Append('s');
        tempComment.Append(' ');
        if (SecondAccessor is not null)
            tempComment.Append("or").Append(' ').Append(SecondAccessor.Pascalize()).Append('s').Append(' ');
